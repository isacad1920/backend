...
class PermissionManager:  # pragma: no cover - deprecated
    """DEPRECATED legacy PermissionManager.

    Retained as a stub to surface clear errors if any stale imports remain.
    All permission logic moved to normalized RBAC utilities in
    `app.core.permissions` and database tables (permission, rolepermission,
    userpermissionoverride).
    """

    def __init__(self, *_, **__):  # noqa: D401
        raise RuntimeError(
            "PermissionManager is deprecated. Use app.core.permissions instead."
        )

    @classmethod
    def __getattr__(cls, item):  # type: ignore
        raise RuntimeError(
            "PermissionManager attribute access deprecated. Use RBAC tables."
        )
